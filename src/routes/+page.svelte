<section class="bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white min-h-screen flex items-center justify-center">
  <div class="max-w-4xl px-6 flex flex-col md:flex-row items-center text-center md:text-left md:gap-6">
    <div class="md:w-1/3 mb-6 md:mb-0">
      <img
        src="/images/fitz_speaker.jpg"
        alt="Fitz Angelo"
        class="w-48 md:w-64 lg:w-72 max-w-full rounded-lg mx-auto md:mx-0 md:mr-6 shadow-lg object-cover"
      />
    </div>
    <div class="md:w-2/3">
      <h1 class="text-4xl md:text-6xl font-bold leading-tight">
        Hi, I'm <span class="text-blue-500">Fitz Angelo</span>
      </h1>
      <p class="mt-4 text-lg md:text-xl text-gray-300 md:whitespace-nowrap">
        A developer with a passion for problem-solving and psychology.
      </p>
      <div class="mt-6 flex justify-center md:justify-start space-x-4">
        <a
          href="#experience"
          class="px-6 py-3 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition"
        >
          See My Work
        </a>
        <a
          href="#contact"
          class="px-6 py-3 bg-gray-700 text-white rounded-lg shadow-md hover:bg-gray-600 transition"
        >
          Contact Me
        </a>
      </div>
    </div>
  </div>
</section>

<section id="experience" class="py-12 bg-gray-100 text-gray-900">
  <div class="max-w-6xl mx-auto px-6">
    <h2 class="text-3xl md:text-4xl font-bold text-center mb-8">My Experience</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div class="p-6 bg-white rounded-lg shadow-lg border border-transparent hover:border-blue-500 hover:shadow-xl transition-all duration-300">
        <h3 class="text-xl font-semibold">Software Developer Intern</h3>
        <p class="text-sm text-gray-600 mt-2">ABC Tech, Jun 2023 - Dec 2023</p>
        <ul class="mt-3 space-y-2 text-gray-700 text-sm">
          <li>• Developed scalable web applications using React.js.</li>
          <li>• Optimized backend performance with Node.js and PostgreSQL.</li>
        </ul>
      </div>
      <div class="p-6 bg-white rounded-lg shadow-lg border border-transparent hover:border-blue-500 hover:shadow-xl transition-all duration-300">
        <h3 class="text-xl font-semibold">Freelance Web Developer</h3>
        <p class="text-sm text-gray-600 mt-2">Jan 2022 - Present</p>
        <ul class="mt-3 space-y-2 text-gray-700 text-sm">
          <li>• Designed and deployed responsive websites for clients.</li>
          <li>• Implemented modern UI/UX principles.</li>
        </ul>
      </div>
      <div class="p-6 bg-white rounded-lg shadow-lg border border-transparent hover:border-blue-500 hover:shadow-xl transition-all duration-300">
        <h3 class="text-xl font-semibold">Hackathon Participant</h3>
        <p class="text-sm text-gray-600 mt-2">Various Events, 2021 - Present</p>
        <ul class="mt-3 space-y-2 text-gray-700 text-sm">
          <li>• Built innovative projects under time constraints.</li>
          <li>• Collaborated with multidisciplinary teams.</li>
        </ul>
      </div>
    </div>
  </div>
</section>

<section id="technologies" class="py-12 bg-gray-900 text-white">
  <div class="max-w-6xl mx-auto px-6">
    <h2 class="text-3xl md:text-4xl font-bold text-center mb-8">Technologies I Use</h2>
    <div class="flex flex-wrap justify-center gap-4">
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        Typescript
      </div>
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        Node.js
      </div>
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        Python
      </div>
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        React
      </div>
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        Svelte
      </div>
      <div class="px-4 py-2 bg-gray-800 rounded-lg shadow-md hover:bg-blue-500 hover:text-white transition-all duration-300">
        PostgreSQL
      </div>
    </div>
  </div>
</section>
